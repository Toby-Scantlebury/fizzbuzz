// Step 16: Inject the fallback presentation policy
#include <iostream>
#include <string>
...
std::string evaluate(int i, const auto& rules, auto fallback)
{
    std::string text;
    for (const auto& rule : rules)
        if (rule.matches(i))
            append_word(text, rule.word);
    return text.empty() ? fallback(i) : text;
}
...
int main()
{
    ...
    auto values = map_sequence(std::views::iota(1, 21),
        [rules](int i) { return evaluate(i, rules, [](int n) { return std::to_string(n); }); });
